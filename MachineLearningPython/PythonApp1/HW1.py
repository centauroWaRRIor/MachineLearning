...
def k_FoldValidation(k, classifier, rows, classification_label):
	...
	for i in range(k):
		...
		print "Training: F1 Score: %.1f , Accuracy: %.1f" % (f1_score, accuracy_score)
	...
	print "Training: F1 Score: %.1f , Accuracy: %.1f" % (numpy.average(f1_score), numpy.average(accuracy_score))
	f1_score = numpy.array(average_f1_test_score)
	accuracy_score = numpy.array(average_accuracy_test_score)
	print "Test: F1 Score: %.1f , Accuracy: %.1f" % (numpy.average(f1_score), numpy.average(accuracy_score))
